    ...
    <aside className="fixed left-0 top-0 z-40 h-screen w-80 backdrop-blur-xl bg-sidebar border-r border-sidebar-border shadow-glass">
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-center justify-between h-16 px-6 border-b border-sidebar-border/50">
          <div className="flex items-center space-x-3">
            <div className="w-10 h-10 rounded-xl shadow-glow flex items-center justify-center">
              <img src={producshineLogoUrl} alt="Producshine" className="w-8 h-8" />
            </div>
        ...
        <nav className="flex-1 px-6 py-4 space-y-2">
          {navigation.map((item) => {
            const requiresAuth = item.name === "Submit Tool" || item.name === "Profile"
            ...
            return (
              <NavLink
                ...
                className={({ isActive }) => {
                  ...
                  const isTrendingActive = item.name === "Trending" && window.location.pathname === "/trending"
                  const isDiscoverActive = item.name === "Discover" && (window.location.pathname === "/discover" || window.location.pathname === "/trending")
                  const actuallyActive = item.name === "Trending" ? isTrendingActive : 
                                       item.name === "Discover" ? (isActive && !isTrendingActive) : 
                                       isActive
